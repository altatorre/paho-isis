...
?>				<?php 
					// cambiojc 20150716
					// Mostrar los modulos asociados al menu. 
				$htmlmsg = "<h2>Associated modules</h2>";
				$db1 = JFactory::getDbo();
				$query2 = "SELECT * FROM #__modules_menu where menuid = 0" . $this->item->id . " limit 30";
				$db1->setQuery($query2);
				$rowmodulesid = $db1->loadObjectList();
				$htmlmsg .= "<ul>";
				foreach ($rowmodulesid as $rowmoduleid) {
						$query1 = "SELECT * FROM #__modules where published=1 and id = " . $rowmoduleid->moduleid . " limit 20";
						$db1->setQuery($query1);
						$rowmodules = $db1->loadObjectList();
						if ($rowmodules)
						$htmlmsg .= "<li>".$rowmoduleid->moduleid.". <b>".$rowmodules[0]->title."</b> <a target=\"_blank\" href=\"index.php?option=com_modules&task=module.edit&id=".$rowmoduleid->moduleid."\">Edit</a><br/>".$rowmodules[0]->module."-".$rowmodules[0]->position."<br />&nbsp;</li>";
					}
				$htmlmsg .= "</ul>";
				echo $htmlmsg;
					// fin de cambio jc
