...
?><?php 
		// cambiojc 20150716 20181106
		// URL del menu
	$string = "https://".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
	$miurl = substr( $string, 0, strpos($string,'administrator')) . $this->item->link;
	$posindex = stripos ( $miurl, '?Itemid' );
	$titulo = "This URL: ";
	if ($posindex == 0) {
				$miurl .= "&Itemid=" . $this->item->id; 
	} else {
		if ($this->item->type == 'alias') { 
			$db2 = JFactory::getDBO();
			if ($this->item->params['aliasoptions'] != '') {
				$query3 = "SELECT id,link FROM #__menu where id = " . $this->item->params['aliasoptions'] . " limit 30";
				$db2->setQuery($query3);
				$rowmodulesid = $db2->loadObjectList();
				$htmlmsg .= "<ul>";
				foreach ($rowmodulesid as $rowmoduleid) {
					$newurl = $rowmoduleid->link."&Itemid=".$rowmoduleid->id;
				}
				$htmlmsg .= "</ul>";
				$posindex2 = stripos ( $miurl, 'index.php' );
				$miurl = substr( $miurl, 0, $posindex2) . $newurl;
				$titulo .= "(redirected)";					
			} else {
				$miurl = "(Not Available)";
			}
	...
	if ($miurl != "(Not Available)")  
		echo "<h2><b>".$titulo."</b> <a target=\"_blank\" href=\"".$miurl."\">".$miurl."</a></h2>";
		/// fin de cambio jc
	 ?>
