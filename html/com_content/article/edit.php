...
?>
<?php 
		// cambio jc. Fragmento que muestra la dirección del artículo.
	$thisurl = JRoute::_('index.php?option=com_content&view=article&id='.$this->item->id.'&Itemid=');
	/* echo JRoute::_(ContentHelperRoute::getArticleRoute($this->item->slug, $this->item->catid, $this->item->language), false); */
		//$thislink = JRoute::_(ContentHelperRoute::getArticleRoute($this->item->slug, $this->item->catid, $this->item->language));
		$thislink = JRoute::_('index.php?option=com_content&view=article&id='.$this->item->id.'&Itemid=');
		$thislink = str_replace("&amp;id=&amp;catid=".$this->item->catid."&amp;", "&amp;id=".$this->item->id."&amp;", $thislink);
		$thislink = "http://".$_SERVER['HTTP_HOST'].str_replace("/administrator/index", "/index", $thislink);

		if ($this->item->state == 1) {
			if (intval($this->item->id) != 0) {
				echo "<strong>This URL: <a href=\"".$thislink."\" target=\"_blank\">".$thislink."</a></strong>"; 
			}	else {
				echo "<strong>This URL: {Article not saved yet}"; 
			}
		} else {
			echo "<strong>This URL: <span style=\"color:red\">{Unpublished article}</span> ".$thislink."</strong>"; 
		} 
			// fin cambio jc
		?>
